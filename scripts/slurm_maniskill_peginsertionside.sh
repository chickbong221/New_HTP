#!/bin/bash
#SBATCH --job-name=htp-ms-peginsertionside
...
# CoRe-WM full (corewm_full) on ManiSkill PegInsertionSide-v1: training, then the
...
# PegInsertionSide-v1 supports only panda_wristcam, whose hand_camera joins
# the env's base_camera: FlattenRGBDObservationWrapper concatenates the two
# into a 6-channel image, exactly as ReLDreamer does, and the evaluation
# figures tile both views side by side. Its pixel MAE therefore averages two
# cameras and is not comparable in absolute terms with PullCubeTool's. The
# second camera doubles the replay to ~59 GB of pixels, close to half of
# --mem=128G. The task registers a 100-step horizon, well above the 33 frames
# the evaluation protocol needs, so no horizon override is passed either.
#
...
echo "Arm: CoRe-WM full (corewm_full), ManiSkill PegInsertionSide-v1"
...
python scripts/smoke_maniskill.py \
  ...
  --logdir $HOME/logdir/New_HTP/preflight_${SLURM_JOB_ID}/PegInsertionSide-v1 || exit 1
...
NAME=PegInsertionSide-v1-corewm_full
...
python -m dreamerv3.main_maniskill \
  ...
  --logger.wandb_group maniskill_PegInsertionSide-v1 \
