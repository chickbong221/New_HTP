...
class ManiSkill(embodied.Env):
  ...
  def __init__(
    ...
    # 0/None -> keep the task's registered horizon; positive -> override it.
    # Eval gets its own value so a longer diagnostic rollout can be taken
    # without changing the training horizon (same knob as ReLDreamer).
    # gymnasium.make() takes max_episode_steps as a named argument and
    # ManiSkill's TimeLimit wrapper reads it from that frame, so the override
    # reaches the env and find_max_episode_steps_value() reads it back.
    ...
    make_kwargs = dict(
        ...
        # By default no max_episode_steps is passed: the registered horizon is
        # used, same as TD-MPC2, and read back with
        # gym_utils.find_max_episode_steps_value.
        **kwargs,
    )
    ...
    # vector episode. The train loop uses this to mirror TD-MPC2
    # final_info_metrics().
  ...
  def obs_space(self):
    spaces = {
        ...
        # make_agent. The train loop reads these only at is_last=True.
  ...
  def step(self, action):
    ...
    Returns obs dict with all values shaped [N, ...]. BatchedDriver._step()
    ...
    if done.any() and 'final_observation' in info:
      ...
      for key in obs:
        if key in final and isinstance(obs[key], torch.Tensor):
          ...
          obs[key] = torch.where(
              expand.expand_as(obs[key]), final[key], obs[key])
  ...
  def _make_obs(self, obs, reward, terminated, truncated, is_first,
    ...
    out = {
        ...
        # steps get zeros; the train loop only reads them at is_last=True.
